    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="flex flex-col items-center justify-center text-center px-6 py-32 gap-6">
        ...
        <div className="flex gap-4 mt-2">
          <Link
            href="/challenges"
            className="flex items-center gap-2 bg-brand hover:bg-brand-dark transition px-6 py-3 rounded-xl font-semibold"
          >
            Browse Challenges <ArrowRight size={16} />
          </Link>
          <Link
            href="/leaderboard"
            className="flex items-center gap-2 border border-white/10 hover:border-brand-light transition px-6 py-3 rounded-xl font-semibold text-gray-300"
          >
            Leaderboard
          </Link>
        </div>
      ...
      <section className="grid md:grid-cols-3 gap-6 px-6 pb-24 max-w-5xl mx-auto w-full">
        {features.map(({ icon: Icon, title, desc }) => (
          <div
            key={title}
            className="bg-card border border-white/5 rounded-2xl p-6 flex flex-col gap-3"
          >
